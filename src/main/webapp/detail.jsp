...
$(".n1").on("click",function(){
	count= parseInt( $(event.target).next().text());
	if(count>1) $(event.target).next().text(""+(--count));
});
$(".n3").on("click",function(){
	count= parseInt($(event.target).prev().text());
	$(event.target).prev().text(""+(++count));
});

$(".goumai2").on("click",function(){
	var pid=$(this).attr("pid");
	var count=$(".n2").text();
	
if(${sessionScope.user!=null}){
	var a=${sessionScope.user.id};
	$.post("addcar",{product_id:pid,count:count,user_id:a},function(res){
		alert(res.text);
	});
	...
	<body>
			...
			<div class="header">
				<div class="header1">
					<div class="header1-cont">
						<div class="left">
							${requestScope.ulist.email}欢迎您来到鲜生购,&nbsp;							
		...
		<div class="container boxs">
			<div class="top fl">
				<div class="overflow">
					<div id="preview" class="spec-preview"> <span class="jqzoom"><img width="521px" height="507px" jqimg="img/gshaf.png" src="img/gshaf.png" /></span> </div>
				</div>
				<!--缩图开始-->
				...
				<li><img width="102px" height="102px" alt="" bimg="${p}" src="${p}" onmousemove="preview(this);"></li>
				</c:forEach>
		...
		<form action="">
			<div class="goods fl">
				<div class="heard">
					珍享  泰国进口金枕头榴莲  一个装  约2kg  顺丰包邮
				</div>
				<div class="hui">
					<div class="money">
						<div class="price">价&nbsp;格:</div>
						<span class="price1">￥${requestScope.info.nowprice}</span>
						<span class="price2"> <del>￥${requestScope.info.price}</del> </span>
					</div>
					<div class="lingquan">
						<div class="quan">活&nbsp;动:</div>
						<span>${requestScope.info.activity}</span>   <!--82*25-->
				...
				<div class="else">
					<div class="number">
						...
						<span class="n1" style="cursor: pointer;">-</span>
						<span class="n2" name="count">1</span>
						<span class="n3" style="cursor: pointer;">+</span>		
						<p class="goumai"><input type="button" class="goumai2"  value="加入购物车" pid="${requestScope.info.id}"></p>
					</div>	
					<div class="tese">
						<div class="tese2">
							特&nbsp;色：
